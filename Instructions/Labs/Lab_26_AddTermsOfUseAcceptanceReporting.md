...
1. 이 연습에서는 가지고 있는 PDF를 선택하거나 Microsoft Word를 사용하여 간단한 사용 약관 문서를 생성한 다음 PDF로 출력할 수 있습니다.

1. 사용 약관 문서의 언어를 선택합니다.

1. 언어 옵션을 사용하면 사용 약관을 다양한 언어로 여러 개 업로드할 수 있습니다. 최종 사용자에게 표시되는 사용 약관의 버전은 브라우저 기본 설정에 기반합니다.
...
1. 선택 창에서 사용 약관 정책을 테스트하는 데 사용할 계정을 선택합니다.

1. 모든 조건부 액세스 정책과 같은 관리자 계정을 선택하는 경우 조건부 액세스 정책을 변경할 수 있는 충분한 권한이 있는 다른 계정이 있는지를 고려해야 합니다. 이는 조건부 액세스 정책으로 인해 원치 않는 결과가 발생하는 경우 관리자 계정이 잠기지 않도록 하기 위한 것입니다.
